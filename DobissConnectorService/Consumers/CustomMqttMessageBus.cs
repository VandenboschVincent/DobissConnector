...
{
    public class CustomMqttMessageBus : MessageBusBase<MqttMessageBusSettings>
    {
        private IManagedMqttClient? _mqttClient;
        private readonly ILogger? _logger;

        public CustomMqttMessageBus(MessageBusSettings settings, MqttMessageBusSettings providerSettings) : base(settings, providerSettings)
        {
        ...
        {
            var consumer = Consumers.Cast<MqttTopicConsumer>().FirstOrDefault(x => Extensions.CheckTopic(x.Path, arg.ApplicationMessage.Topic));
            if (consumer != null)
            {
                var headers = new Dictionary<string, object>
                {
                    { "origPath", arg.ApplicationMessage.Topic }
                };
